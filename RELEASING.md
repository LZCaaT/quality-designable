...
2. 使用 Node.js 22 安装依赖；`preversion` 会运行测试、构建和只读 lint 检查。
3. 执行 `npm run version:alpha -- --yes --no-push`，生成发布提交和 `v<version>` 标签。
4. 推送发布提交，再推送标签。标签触发 `.github/workflows/publish.yml`。
